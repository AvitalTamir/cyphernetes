...
func fetchOpenAPISpec() (*openapi_v2.Document, error) {
...
func parseOpenAPISpec(data []byte) (*openapi_v2.Document, error) {
	doc := &openapi_v2.Document{}
	err := proto.Unmarshal(data, doc)
	if err != nil {
		return nil, fmt.Errorf("failed to unmarshal OpenAPI spec: %v", err)
	}
	return doc, nil
}

func getOpenAPIDocument() (*openapi_v2.Document, error) {
	schema, err := fetchOpenAPISpec()
	if err != nil {
		return nil, err
	}

	doc, err := parseOpenAPISpec(schema.ToRawInfo())
	if err != nil {
		return nil, err
	}
