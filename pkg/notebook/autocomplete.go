...
func (c *CyphernetesCompleter) Do(line []rune, pos int) ([][]rune, int) {
	...
	if len(words) > 0 {
		...
		if resourceKindIdentifierRegex.MatchString(lastWord) {
			...
			resourceKinds := c.getResourceKinds(identifier)
		...
		} else if c.isJSONPathContext(lineStr, pos, lastWord) {
			identifier := strings.Split(lastWord, ".")[0]
			kind := c.getKindForIdentifier(lineStr, identifier)
			treeStructure, err := c.fetchResourceTreeStructureForKind(kind)
...
func (c *CyphernetesCompleter) fetchResourceTreeStructureForKind(kind string) ([]string, error) {
	if c.executor == nil {
		return nil, fmt.Errorf("executor not initialized")
	}

	schemaName := c.getSchemaName(kind)
	if schemaName == "" {
...
func (c *CyphernetesCompleter) getSchemaName(kind string) string {
...
func (c *CyphernetesCompleter) getKindForIdentifier(line string, identifier string) string {
	regex := regexp.MustCompile(`\((\w+):(\w+)`)
	matches := regex.FindAllStringSubmatch(line, -1)
	for _, match := range matches {
		if match[1] == identifier {
			kind := match[2]
			if c.executor != nil && c.executor.executor != nil {
				apiProvider, ok := c.executor.executor.Provider().(*apiserver.APIServerProvider)
				if !ok {
					return kind
				}
				...
				if gvr, ok := cache[strings.ToLower(kind)]; ok {
					return c.findCanonicalKind(cache, gvr)
				}
...
func (c *CyphernetesCompleter) findCanonicalKind(gvrCache map[string]schema.GroupVersionResource, gvr schema.GroupVersionResource) string {
...
func (c *CyphernetesCompleter) getResourceKinds(identifier string) []string {
	if c.executor == nil {
		return nil
	}
...
func (c *CyphernetesCompleter) isJSONPathContext(line string, pos int, lastWord string) bool {
